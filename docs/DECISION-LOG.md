...
### DEC-047 — Attachment upload had a genuine production bug; the task-edit form did not (it was just slow) — both simplified anyway
**Finding (genuine bug, `uploadAttachment`):** `TaskAttachments` called `formRef.current?.requestSubmit()` from the file input's `onChange` to submit a `<form action={uploadAction}>` (a `useActionState`-bound action). In production, selecting a file correctly populated `input.files`, but **zero network requests for the upload action ever fired** — confirmed directly via request-listener instrumentation, not guessed. The same interaction worked in local dev. Root cause not fully bisected (likely a React-production-build/Radix-Portal interaction with synthetic `requestSubmit()` calls specifically), but the fix is solid regardless: call `uploadAttachment` directly from the `onChange` handler via `startTransition`, building `FormData` manually — the same pattern every other mutation in this app already uses successfully.
**Correction (false alarm, `editTask`):** Investigating Priority 8's progress field initially suggested the same failure for the main task-edit form ("Save" button): the database showed no update after clicking Save and reloading. This led to converting `editTask` away from `<form action={fn}>` + `useActionState` too. **Re-verifying with a longer wait (15s instead of ~2s) showed the original suspicion was wrong** — `editTask` does run and does return success, it just took roughly 7 seconds end-to-end in production (consistent with serverless cold start + cross-region Supabase round-trip), and every prior verification attempt simply hadn't waited long enough before checking the database. This was a real lesson in this project's own "collect evidence first" rule: the first failure-shaped result was treated as conclusive before ruling out "it just hasn't finished yet."
**Decision:** Kept the `editTask` simplification (direct call via `startTransition`, no `useActionState`) since it's a reasonable cleanup matching the codebase's dominant pattern and is now confirmed working correctly with adequate wait time — but the DECISION-LOG should not claim it fixed a bug that wasn't actually there. The genuine fix in this entry is `uploadAttachment`'s.
**Tradeoffs:** None functionally. The `editTask` change did remove a `handledEditState` ref-guard effect that's no longer needed with a direct call.
**Owner:** Product Engineer
**Date:** 2026-06-29 (Sprint 4)
...
A new workspace-level page (`/[workspaceSlug]/calendar`, linked from the sidebar's Workspace section) aggregates task due dates across **every** project in the workspace (not just one), workspace-scoped `meetings` (new `meetings`/`meeting_attendees` tables, migration 034 — mirrors `task_assignees`' join-table RLS shape but rooted at `is_workspace_member` via a new `is_workspace_member_for_meeting()` helper instead of `is_workspace_member_for_task`), and **milestones**, which deliberately reuse the existing `projects.due_date` column (added in migration 031 for the project header) rather than introducing a new "milestone" entity — a project already has exactly one due date, and the brief's "milestones" requirement is fully satisfied by surfacing that on the calendar. Milestone chips are read-only (click navigates to the project); they are not draggable, since dragging a project's own due date from a workspace-wide calendar felt like the wrong place for that edit.
**Rationale:** "Reuse existing calendar infrastructure wherever possible" was explicit in this priority's brief — extending one component over forking a parallel one was the lower-complexity path, and the optional-props design means the per-project tab pays zero cost for capabilities it doesn't use. Reusing `projects.due_date` for milestones avoids a schema addition the brief didn't actually require once the existing column was found.
**Tradeoffs:** Meeting attendee updates are implemented as delete-all-then-reinsert rather than a diff (`updateMeeting`) — acceptable given meetings have few attendees and this mirrors no existing diff-based pattern in the codebase to deviate from. Meetings do not generate notifications (unlike task assignment) — the `notifications.type` check constraint would need a migration to add a `meeting_invited` type, which felt like scope beyond "minimum scope" for this priority; logged as a gap below.
**Bug found and fixed during verification:** The initial migration created `meetings`/`meeting_attendees` with RLS policies but no table-level `grant ... to authenticated` — Postgres requires both (RLS narrows an already-granted privilege; it doesn't substitute for one), unlike `task_assignees` (migration 033) which had the grant. Confirmed directly via a production-shaped local repro: creating a meeting failed with `permission denied for table meetings` even though the policies were correct. Fixed by amending migration 034 and applying the missing grants live before commit.
**Owner:** Product Engineer
**Date:** 2026-06-29 (Sprint 4)
**Future Revisit Conditions:** If a future migration adds a new RLS-protected table, check for the matching `grant` statement before considering it done — RLS-without-grant is a silent-until-tested failure mode that bit this exact priority. If meeting notifications are wanted later, add a `meeting_invited` (or similar) value to `notifications.type`'s check constraint rather than overloading an existing type.
...
`tasks.assignee_id` (single, nullable) was **not** dropped or migrated away from — 12 files across notifications, the dashboard's "assigned to you" widget, board sort-by-assignee, and the calendar already depend on it, and rewriting all of those was out of scope for this priority. It now means "primary assignee" and is kept in sync by the new actions: set when the first assignee is added (`ensurePrimaryAssignee`), reassigned to a remaining assignee or cleared when the primary is removed (`reassignPrimaryAfterRemoval`). `editTask` no longer touches `assignee_id` at all — it would otherwise clobber the picker-managed value on the next unrelated save (e.g. changing the title), since its old code unconditionally wrote `assigneeId || null` from a form field that no longer exists.
**Rationale:** A join table is the standard, already-precedented shape for a many-to-many relationship in this codebase (DEC mirrors `task_labels`) — there was no reason to invent a different pattern. Preserving `assignee_id` as "primary assignee" rather than rewriting every consumer was the explicit instruction ("preserve existing permissions," "do not rewrite working architecture") and the lower-risk choice: the alternative (rewriting notifications/dashboard/sort/calendar to read from the join table) touches code with its own established, tested behavior for no functional gain this priority asked for.
**Tradeoffs:** `TaskTableView`'s "Assignee" column and the calendar still show only the primary assignee, not the full list — acceptable for now since neither was named in this priority's scope (Kanban card stacked avatars specifically), but worth revisiting if multi-assignee visibility is wanted everywhere. Three existing Playwright tests (`board-power-features`, `dashboard`, `notifications`) used `getByLabel("Assignee").selectOption(...)` against the now-removed select; updated to click the picker's toggle button instead, in the same test runs that already covered this functionality (no new tests added beyond that, since assigning is a separate immediate action now rather than part of the form's Save submission).
**Owner:** Product Engineer
**Date:** 2026-06-29 (Sprint 4)
**Future Revisit Conditions:** If a pilot org needs `TaskTableView`/calendar to show every assignee (not just the primary), thread `assigneeIds` into those views the same way `kanban-column.tsx`/`task-card.tsx` already do. If `assignee_id`'s "primary assignee" semantics ever feel confusing in practice, consider a dedicated `is_primary` flag on `task_assignees` instead of inferring it from insertion order.

---

### DEC-049 — Sprint 4 Phase 0 Bug 1: progress calculation ignored task status; moveTask's known "no revalidatePath" gap now has a real consequence
**Finding:** Two distinct, confirmed bugs, found by direct reproduction (not guessed) across the full data flow — database → types → actions → calculation → rendering:
1. **Calculation bug:** `getTaskProgress()` (the shared helper added in DEC-046) computed progress from checklist-completion-or-manual-percentage only, with no awareness of the task's `status`. A task with no checklist, moved to the Done column without ever touching the manual progress slider, kept reading as 0% — confirmed directly: the completion sidebar showed the contradictory "1/1 tasks done" (status-based count, correct) next to "PROGRESS 0%" (the same helper, wrong) on the same screen. Worse, `TaskCard` had never been switched to use the shared helper at all — it duplicated the checklist-or-manual logic inline, which is exactly how a fix in one place (DEC-046) failed to reach another.
2. **Staleness bug:** `moveTask` has a long-standing, deliberate, *documented* decision (in its own code comment, predating this sprint) to skip `revalidatePath` because it's called directly from drag-end/the keyboard move control rather than a `<form>` action, so Next.js's automatic post-Server-Action route refresh doesn't apply, and "the caller's own optimistic state is already the source of truth for this view." That reasoning was correct when written, but Sprint 4 Priorities 3/5/8 added `ProjectHeader` and `ProjectCompletionSidebar`, which read project-wide aggregates as server-rendered props — a "view" `moveTask` doesn't know about and never refreshes. Confirmed directly: moving a task to Done via the keyboard control left the header frozen at its pre-move values indefinitely (not just slow — no amount of waiting fixed it, unlike DEC-047's `editTask` finding).
**Decision:** `getTaskProgress`/`getAverageProgress` (`lib/utils/task-progress.ts`) now take `status` and return 100 whenever `status === "done"`, before checking checklist/manual progress. `TaskCard` was switched to call the shared helper instead of its own duplicate logic. `KanbanBoard`'s `commitMove` (the single function both drag-and-drop and the keyboard control funnel through) now calls `router.refresh()` after a successful move.
**Rationale:** "Done means 100%" is the obviously-intended semantic and matches how every other task-management tool treats status — checklist/manual progress are proxies for completion, not the only sources of truth once a card is explicitly marked done. The `router.refresh()` placement at `commitMove` (rather than inside `moveTask` itself) keeps the fix in the client layer that actually knows it has a server-rendered sibling to refresh, without changing `moveTask`'s own behavior or its now-partially-outdated comment's reasoning for *why* it skips revalidation server-side.
**Tradeoffs:** Every task move now triggers one additional RSC refetch. Acceptable — it's a single lightweight request per move, and this codebase already does the same after edit/archive/delete in `TaskDetailDialog`.
**Owner:** Product Engineer
**Date:** 2026-06-29 (Sprint 4, Phase 0)
**Future Revisit Conditions:** If a future view reads project-wide aggregates from anywhere else, audit it against `moveTask`'s comment first — that comment is the tell that this class of staleness can recur. Don't add `router.refresh()` speculatively to actions that already call `revalidatePath` (editTask, archive/delete) — confirmed in this same investigation that those self-correct via Next.js's automatic refresh, just with some latency (DEC-047); only `moveTask` has zero self-correction.
