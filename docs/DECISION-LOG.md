...
### DEC-047 — Critical: `<form action={fn}>` + `useActionState` inside TaskDetailDialog silently never reached the server in production
**Finding:** While verifying Priority 8's manual progress field in production (not just locally — see "collect evidence first"), the slider correctly reached 40% client-side (confirmed via the DOM value and a `new FormData(form)` read showing `progress: "40"`), but the database still showed `progress: 0` and `updated_at === created_at` (i.e. no update had ever landed) after clicking Save and reloading. Widening the test to a plain title rename on the same dialog reproduced the identical failure: editing a task's title via the main "Save" button **did not persist in production at all**, while the exact same interaction worked correctly in local dev and in this project's existing Playwright suite (which only ever runs against `localhost`, never against the deployed URL — explaining why this had never been caught). `TaskAttachments`' upload had the same symptom, already root-caused earlier in this session via a different trigger path (`formRef.current?.requestSubmit()` from an unrelated input's `onChange`). Both forms live inside this same Radix `Dialog`'s portaled content and both used `<form action={fn}>` + `useActionState`. Every *working* mutation in this app either lives in a plain (non-portaled) page-level form (sign-up, sign-in — confirmed fine in production) or calls its Server Action directly from a plain `onClick`/`onChange` handler via `startTransition` (rename, archive, delete, checklist, comments, labels — all confirmed fine in production). The common factor in the two broken cases was specifically "form-action interception, inside this dialog's portal, in a production build."
**Decision:** Converted both `editTask` (the task detail form) and `uploadAttachment` (file input) away from `<form action={fn}>` + `useActionState` to plain handlers that build `FormData` manually and call the Server Action directly inside `startTransition` — exactly the pattern every other mutation in this app already uses successfully. `editTask`'s submit handler does `event.preventDefault()` then constructs `FormData` from `event.currentTarget`; the upload handler builds `FormData` from the selected `File` directly.
**Rationale:** This is the proven-working pattern elsewhere in this exact codebase, not a novel fix — converting to match it sidesteps whatever specific interaction between Next.js's production-optimized form-action interception and Radix's Portal was causing the silent failure, without needing to root-cause that interaction precisely (which would require bisecting Next.js/React internals neither owned by this app). Per this project's debugging rule ("collect evidence first, fix second"), the failure was reproduced and measured (DOM, FormData, and DB state all directly inspected) before any code changed.
**Tradeoffs:** None — the new handlers are simpler than the `useActionState` versions they replace (no `handledEditState` ref-guard effect needed, no remount-via-`key` dance required for the action result).
**Owner:** Product Engineer
**Date:** 2026-06-29 (Sprint 4)
**Future Revisit Conditions:** Treat any new `<form action={fn}>` + `useActionState` pair *inside a Dialog* as suspect until verified in production specifically, not just locally — this project's e2e suite cannot catch this class of bug since it only runs against localhost. If a third instance of this exact failure mode turns up, it's worth filing upstream against Next.js/Radix rather than continuing to patch call sites one at a time.
